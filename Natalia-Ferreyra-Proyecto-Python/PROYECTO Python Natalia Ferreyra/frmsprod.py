...
class Producto(Toplevel):
    def __init__(self, master=None, isAdmin=False, producto_id=None):
        ...
        GLabel_737["anchor"] = "e"
        GLabel_737["text"] = "Categoria:"
        GLabel_737.place(x=10, y=130, width=121, height=30)

        GLineEdit_234 = Entry(self, name="txtCategoria")
        ...
        GLabel_616["anchor"] = "e"
        GLabel_616["text"] = "Precio:"
        GLabel_616.place(x=10, y=210, width=123, height=30)

        GLineEdit_481 = Entry(self, name="txtPrecio")
        ...
        GLabel_61["fg"] = "#333333"
        GLabel_61["anchor"] = "e"
        GLabel_61["text"] = "ProductoId:"
        GLabel_61.place(x=10, y=250, width=124, height=30)

        GLineEdit_366 = Entry(self, name="txtProductoId")
        ...
        if producto_id is not None:
            producto = product.obtener_id(producto_id)
            if producto is None:
                tkMsgBox.showerror(
                    self.master.title(),
                    "Se produjo un error al obtener los datos del producto, reintente nuevamente",
                )
                self.destroy()
            else:
                ...
                GLineEdit_481.insert(0, producto[6])
                GLineEdit_366.insert(0, producto_id)
                GLineEdit_366["state"] = "disabled"
            #    cb_roles.set(producto[8])
    ...
    def aceptar(self):
        ...
        producto_Id = self.get_value("txtProductoId")
        ...
        if producto_Id is None:
            ...
            if not product.existe(Nombre):
                product.agregar(
                    Nombre, Descripcion, Marca, Categoria, Cantidad, Precio, producto_Id
                )
                ...
                tkMsgBox.showwarning(
                    self.master.title(), "Producto existente en nuestros registros"
                )
        else:
            print("Actualizacion de producto")
            product.actualizar(
                Nombre, Descripcion, Marca, Categoria, Cantidad, Precio, producto_Id
            )  # TODO ver el tema de la contraseña
